    ...
    <div className="modal-overlay open" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-meta">
          <div className="modal-meta-item"><label>{t('Employment Type', '雇用形態')}</label><span>{job.type}</span></div>
          <div className="modal-meta-item"><label>{t('Location', '勤務地')}</label><span>{job.loc}</span></div>
          <div className="modal-meta-item"><label>{t('Salary', '年収')}</label><span>{job.salary}</span></div>
        </div>
        {job.body.split('\n\n').map((para, i) => (
          <div className="modal-section" key={i}>{i === 0 && <h4>{t('Job Description', '仕事内容')}</h4>}<p>{para}</p></div>
        ))}
        {job.required.length > 0 && (
          <div className="modal-section"><h4>{t('Required Skills', '必須スキル')}</h4><ul>{job.required.map(r => <li key={r}>{r}</li>)}</ul></div>
        )}
        {job.welcome.length > 0 && (
          <div className="modal-section"><h4>{t('Nice to Have', '歓迎スキル')}</h4><ul>{job.welcome.map(w => <li key={w}>{w}</li>)}</ul></div>
        )}
...
export default function Careers() {
  ...
  const filtered = filter === 'all' ? jobList : jobList.filter(j => j.filter === filter)
  const FILTERS = [['all', t('All', 'All')], ['engineering', 'Engineering'], ['design', 'Design'], ['easy', t('Easy roles', 'イージー系')]]
    ...
    <>
      ...
      <div className="wrap">
        <div className="section-block">
          ...
          <h2 className="section-heading fade-in">{t('Application Requirements (all roles)', '応募条件（全職種共通）')}</h2>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(260px, 1fr))',gap:'1px',background:'var(--border)',border:'1px solid var(--border)'}} className="fade-in">
            <div style={{background:'var(--black)',padding:'3rem 2.5rem'}}>
              <div style={{fontSize:'0.68rem',letterSpacing:'0.25em',textTransform:'uppercase',color:'var(--yellow)',marginBottom:'1.5rem',opacity:0.8}}>{t('Required', '必須条件')}</div>
              <ul style={{listStyle:'none',display:'flex',flexDirection:'column',gap:'1rem'}}>
                {requirements[lang].required.map(r => (
            ...
            <div style={{background:'var(--black)',padding:'3rem 2.5rem',borderLeft:'1px solid var(--border)'}}>
              <div style={{fontSize:'0.68rem',letterSpacing:'0.25em',textTransform:'uppercase',color:'var(--yellow)',marginBottom:'1.5rem',opacity:0.8}}>{t('Nice to Have', '歓迎条件')}</div>
              <ul style={{listStyle:'none',display:'flex',flexDirection:'column',gap:'1rem'}}>
                {requirements[lang].welcomed.map(r => (
                  <li key={r} style={{display:'flex',gap:'1rem',alignItems:'flex-start',fontSize:'0.92rem',opacity:0.75}}>
                    <span style={{color:'var(--yellow)',flexShrink:0}}>◎</span>{r}
                  </li>
                ))}
          ...
          <p style={{marginTop:'1.5rem',fontSize:'0.72rem',opacity:0.3,fontStyle:'italic'}}>{t('* Education, work history, certifications, language skills, and technical skills are all "nice if you have them." Come easily.', '※ 学歴・職歴・資格・語学力・技術スキルについては、「あれば嬉しい」程度です。イージーに来てください。')}</p>
        </div>
      </div>
      ...
      <div className="wrap">
        <div className="section-block">
          ...
          <h2 className="section-heading fade-in">{t('Selection Process', '選考プロセス')}</h2>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(280px, 1fr))',gap:'1px',background:'var(--border)',border:'1px solid var(--border)'}} className="fade-in">
            {process[lang].map(({ num, title, desc, time }) => (
          ...
          <p style={{marginTop:'1.5rem',fontSize:'0.72rem',opacity:0.3,fontStyle:'italic'}}>{t('* Even if we pass on you, we\'ll tell you as easily as we can: "it wasn\'t meant to be — go easily on to the next thing."', '※ 不採用の場合も、できる限りイージーにお伝えします。「ご縁がありませんでした。イージーに次へ行ってください」とお伝えします。')}</p>
        </div>
      </div>
      ...
      <div className="wrap">
        <div className="section-block">
          ...
          <h2 className="section-heading fade-in">{t('Benefits (easily generous)', '待遇（イージーに充実）')}</h2>
          <div className="perks-grid">
            {benefits[lang].map(({ icon, title, desc }) => (
      ...
      <div className="wrap">
        <div className="section-block">
          ...
          <h2 className="section-heading fade-in">{t('Open Positions', '募集中のポジション')}</h2>
          <p style={{fontSize:'0.8rem',opacity:0.4,fontStyle:'italic',marginBottom:'2rem'}}>{t('* The job description for every role is "make it easy." Only the department changes — the work stays the same.', '※ 全職種の仕事内容は「イージーにすること」です。部署が違うだけで、やることは同じです。')}</p>
      ...
      <div className="wrap">
        <div className="section-block" style={{textAlign:'center'}}>
          ...
          <h2 className="fade-in" style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:'clamp(1.8rem,3vw,2.6rem)',marginBottom:'1.5rem',textAlign:'center'}}>{t("Don't like any of these positions?", 'ポジションが気に入らない？')}</h2>
          <p className="fade-in" style={{opacity:0.5,margin:'0 auto 3rem',fontSize:'0.95rem',lineHeight:1.85}}>{t('If you have something you want to do, easily, just send us that feeling. We\'ll figure out the position title together.', '「イージーにやりたいことがある」という方は、その気持ちだけ送ってください。ポジション名は一緒に考えます。')}</p>
          <a href="mailto:careers@easy-inc.jp" className="fade-in" style={{display:'inline-block',background:'var(--yellow)',color:'var(--black)',fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:'0.85rem',letterSpacing:'0.14em',textTransform:'uppercase',padding:'1rem 2.5rem',textDecoration:'none'}}>
            {t('Get in touch, easily →', 'イージーに連絡する →')}
          </a>
        </div>
      </div>

      <Modal job={modal} onClose={() => setModal(null)} t={t} />
    </>
  )
}
